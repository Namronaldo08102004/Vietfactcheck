...
def run_retrieval_cer_experiment():
    ...
    # Tải dữ liệu để đánh giá (Chỉ đánh giá trên tập Train)
    ...
    # 5. Vòng lặp đánh giá
    for record in tqdm(train_records, desc="Evaluating CER Accuracy"):
        query = record["Statement"]
        ground_truth_url = record["Url"]

        # Bước A: Retrieval - Lấy Top 3 URLs sử dụng trọng số tùy chỉnh
        top_3_urls = retrieval_module.get_top_k_url(query, top_k = 3, weights = CURRENT_WEIGHTS)
        ...
        results_log.append({
            ...
            "top_3_before_rerank": top_3_urls
        })

    # 6. Tính toán và lưu kết quả
    accuracy = (correct_count / total_count) * 100
    print("\n" + "="*50)
    print(f"RESULTS FOR: Retrieval + CER (Top-3 Unified Index)")
    print(f"Final Top-1 Accuracy: {accuracy:.2f}% ({correct_count}/{total_count})")
    print("="*50)

    output_log = os.path.join(current_dir, "results", "retrieval_and_cer_results.json")
