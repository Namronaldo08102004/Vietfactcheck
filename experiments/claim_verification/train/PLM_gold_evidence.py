...
def run_all_plm_gold_evidence():
    ...
    data_path = os.path.join(project_root, "data", "verification", "train.json")
    ...
    for model_path in plm_models:
        ...
        try:
            ...
            for record in tqdm(records, desc = f"Processing {model_name_safe}"):
                ...
                gold_evidence = str(record.get("Evidence", ""))
            ...
            # 3. LƯU FILE KẾT QUẢ
            output_dir = os.path.join(project_root, "experiments", "claim_verification", "train", "results")
            os.makedirs(output_dir, exist_ok=True)
            output_file = os.path.join(output_dir, f"results_gold_evidence_{model_name_safe}.json")
